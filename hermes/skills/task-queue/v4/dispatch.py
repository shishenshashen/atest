...
def _hermes_cli_exists() -> bool:
    """兼容旧 API：等价于 cli_detect.get_best_cli() is not None"""
    try:
        import cli_detect as tq_cli_detect
        return tq_cli_detect.get_best_cli() is not None
    except Exception:
        return shutil.which("hermes") is not None


def _spawn_subagent(rendered_goal: str, task_id: str,
                     heartbeat_sec: int, timeout_sec: int,
                     sync: bool = False,
                     probe_only: bool = False) -> tuple:
    """
    按 fallback 链选 CLI 派发子 agent。
    返回 (cli_name, subprocess_result_or_Popen)。全失败返回 (None, None)。

    probe_only=True: 只检测 CLI 是否可用，**不执行命令**。返回 (cli_name, None) 或 (None, None)。
    """
    import cli_detect as tq_cli_detect
    best = tq_cli_detect.get_best_cli()
    if not best:
        return (None, None)

    if best == "hermes":
        cmd = ["hermes", "delegate", "run",
               "--goal", rendered_goal,
               "--task-id", task_id,
               "--heartbeat", str(heartbeat_sec)]
    elif best == "claude":
        cmd = ["claude", "--print", rendered_goal]
    elif best == "codex":
        cmd = ["codex", "--quiet", rendered_goal]
    elif best == "cursor-agent":
        cmd = ["cursor-agent", "--print", rendered_goal]
    elif best == "aider":
        cmd = ["aider", "--message", rendered_goal, "--no-auto-commits"]
    else:
        return (None, None)

    if probe_only:
        # 只确认 CLI 存在即可（shutil.which 已经做过，这里再保险一次）
        first = cmd[0]
        if not (shutil.which(first) or shutil.which(first + ".exe")):
            # 缓存过期，强制刷新
            caps = tq_cli_detect.detect(force_refresh=True)
            if caps.get("best") and caps["best"] != best:
                return _spawn_subagent(rendered_goal, task_id, heartbeat_sec,
                                       timeout_sec, sync, probe_only)
            return (None, None)
        return (best, None)
...
# v4: 旧 _hermes_cli_exists 保留兼容；新代码用 _spawn_subagent
...
def dispatch(goal: str, agent_role: str = "general",
    ...
    # P1-5: 验证 CLI 在 PATH（cli_detect fallback 链，probe_only 不执行命令）
    cli_name, _ = _spawn_subagent(rendered_goal, task["task_id"],
                                   task["heartbeat_sec"], task["timeout_sec"],
                                   probe_only=True)
    if cli_name is None:
        # 立即标 failed + push 告警
        tq.mark_failed(
            task["task_id"],
            error="no agent CLI available in PATH",
            reason="dispatch_no_cli",
        )
        tq_push.push(
            f"❌ 派发失败: {task['task_id']}\n"
            f"原因: 没有任何 agent CLI 可用 (hermes/claude/codex/cursor-agent/aider 全失败)\n"
            f"目标: {goal[:100]}\n"
            f"修法: 装一个，或设 HERMES_BIN / CLAUDE_BIN 等环境变量",
            level="error",
        )
        raise DispatchError(
            f"no agent CLI available; task {task['task_id']} marked failed"
        )

    # CLI 可用：用 cli_name 实际派发（async 模式）
    cli_name, proc = _spawn_subagent(rendered_goal, task["task_id"],
                                       task["heartbeat_sec"], task["timeout_sec"],
                                       sync=sync)
    if proc is None:
        # 竞态：探测时可用，派发时不可用
        tq.mark_failed(task["task_id"],
                       error="dispatch race: CLI became unavailable",
                       reason="dispatch_race")
        tq_push.push(
            f"❌ 派发竞态失败: {task['task_id']}\n"
            f"CLI 探测时可用，派发时不可用",
            level="error",
        )
        raise DispatchError("dispatch race")

    cmd_desc = f"{cli_name} delegate ({'sync' if sync else 'async'})"
    ...
    try:
        if sync:
            ...
            if result.returncode != 0:
                tq.mark_failed(
                    task["task_id"],
                    error=f"{cmd_desc} exit {result.returncode}: "
                          f"{result.stderr[:500]}",
                    reason="dispatch_nonzero_exit",
                )
                tq_push.push(
                    f"❌ 派发失败: {task['task_id']}\n"
                    f"{cmd_desc} 退出码 {result.returncode}\n"
                    f"stderr: {result.stderr[:300]}",
                    level="error",
                )
                raise DispatchError(
                    f"{cmd_desc} exit {result.returncode}"
                )
            tq.mark_done(task["task_id"],
                         result={"cli": cli_name,
                                 "raw": result.stdout[-2000:]})
        else:
            # async 模式: Popen 已经启动，task 留给子 agent 心跳 + finalize
            pass
